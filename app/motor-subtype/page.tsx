  ...
  useEffect(() => {
    ...
    const fetchSubtypes = async () => {
      try {
        ...
        if (!response.ok) throw new Error(data.message || "Failed to fetch motor subtypes");
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-white via-[#edf4f7] to-[#f4f9fb]">
      ...
      <div className="pt-24 pb-20 px-4 md:px-16 flex-grow">
        {loading ? (
          ...
          <p className="text-center text-gray-500">No motor subtypes available.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {subtypes.map((item, index) => {
              ...
              return (
                ...
                >
                  <div>
                    <h2 className="text-lg font-semibold text-[#397397] uppercase">{product.name}</h2>
                    <p className="text-sm text-gray-600 mt-1 mb-3">{product.description}</p>

                    <ul className="text-sm text-gray-700 space-y-1">
                      <li><strong>Rate:</strong> {rate?.rate ?? "N/A"}%</li>
                      <li><strong>Subtype:</strong> {product.subtype}</li>
                      <li>
                      ...
                      {product.tonnes?.length > 0 && (
                        <li>
                          <strong>Tonnage:</strong> {product.tonnes.join(", ")} tons
                        </li>
                      )}
                    </ul>
                  </div>
